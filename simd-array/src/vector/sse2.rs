use std::arch::x86_64::{
    ...
    _mm_cmpgt_ps, _mm_cmplt_pd, _mm_cmplt_ps, _mm_cvtps_epi32, _mm_div_pd, _mm_div_ps, _mm_load_pd,
    _mm_load_ps, _mm_load_si128, _mm_loadu_pd, _mm_loadu_ps, _mm_max_pd, _mm_max_ps, _mm_min_pd,
    _mm_min_ps, _mm_mul_pd, _mm_mul_ps, _mm_or_pd, _mm_or_ps, _mm_set1_pd, _mm_set1_ps,
...
#[derive(Default)]
pub struct SSE2Vector32;

impl SimdVector for SSE2Vector32 {
    ...
    unsafe fn floor(a: Self::Float) -> Self::Float {
        // TODO: replace by floor implemented using SIMD
        let mut data_f32: Aligned<A16, _> = Aligned([0f32; 4]);
        _mm_store_ps(data_f32.as_mut_ptr(), a);
        data_f32.iter_mut().for_each(|v| *v = v.floor());
        _mm_load_ps(data_f32.as_ptr())
    }
...
#[derive(Default)]
pub struct SSE2Vector64;

impl SimdVector for SSE2Vector64 {
    ...
    unsafe fn floor(a: Self::Float) -> Self::Float {
        ...
        _mm_store_pd(data_f64.as_mut_ptr(), a);
        data_f64.iter_mut().for_each(|v| *v = v.floor());
        _mm_load_pd(data_f64.as_ptr())
    }
